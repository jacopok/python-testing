...
class dirty_matrix():
    def __init__(self, A, b, eps=1e-5):
        self.A = A
        self.b = b
...
def gauss_seidel(A, b, ansatz=None, eps = 1e-10, relaxation=True, verbose=True, diagonally_dominant=False):
    ...
    if(not diagonally_dominant):
        if (not_diagonally_dominant(A)):
            ...
            iterator = dirty_matrix(A, b)
            xarray = []
            narray = []
            from tqdm import tqdm
            for _ in tqdm(range(10000)):
                x, n = gauss_seidel(*next(iterator), ansatz=ansatz, relaxation=relaxation, verbose=False, diagonally_dominant=True)
                xarray.append(x)
                narray.append(n)
