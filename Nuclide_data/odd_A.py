...
plt.plot(A_test, B_lin(A_test, a_lin), label = 'Z*Z')
plt.plot(A_test, B_cor(A_test, a_cor), label = 'Z*(Z-1)')
plt.xlabel('$A^{2/3}$')
plt.ylabel('$B$ difference between $Z = (A\pm1)/2$')
plt.legend()
fit.savefig('odd_A_fit.eps', format='eps')

res = plt.figure(2)
plt.scatter(A23_arr, B_lin(A23_arr, a_lin) - B_arr, label = 'Z*Z')
plt.scatter(A23_arr, B_cor(A23_arr, a_cor) - B_arr, label = 'Z*(Z-1)')
plt.plot(A23_arr, np.zeros(len(A23_arr)))
plt.xlabel('$A^{2/3}$')
plt.ylabel('$B$ difference between $Z = (A\pm 1)/2$ minus model')
plt.legend()
res.savefig('odd_A_residuals.eps', format='eps')
